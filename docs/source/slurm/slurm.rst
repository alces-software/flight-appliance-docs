...
In order to promote efficent usage of the cluster - the job-scheduler is automatically configured with default run-tmie limits for jobs. These defaults can be overridden by users to help the scheduler understand how you want it to run your job. If we don't include any instructions to the scheduler then the default limits are applied to a job.
...
Including job scheduler instructions in your job-scripts is often the most convinient method of working for batch jobs - follow the guidelines below for the best experience:
 ...
 - Instructions are parsed at job subsmission time, before the job itself has actually run. This means you can't, for example, tell the scheduler to put your job output in a directory that you create in the job-script itself - the directory will not exist when the job starts running, and your job will fail with an error.
...
Your cluster job scheduler automatically creates a number of pseudo environment variables which are available to your job-scripts when they are running on cluster compute nodes, along with standard Linux variables. Useful values include the following:
 ...
 For example; the following job-script includes a ``--output=[file_name]`` instruction to set the output file location:
...
A common workload is having a large number of jobs to run which basically do the same thing, aside perhaps from having different input data. You could generate a job-script for each of them and submit it, but that's not very convenient - especially if you have many hundreds or thousands of tasks to complete. Such jobs are known as **task arrays** - an `embarrisingly parallel <https://en.wikipedia.org/wiki/Embarrassingly_parallel>`_ job will often fit into this category.
...
.. code:: bash
  ...
  #SBATCH -D /home/alces/
  #SBATCH --output=output.array
  #SBATCH --array=1-1000
  echo "I am $SLURM_ARRAY_TASK_ID" >> output.array
...
.. note:: If you request more CPU cores than your cluster can accomodate, your job will wait in the queue (in case more nodes are added to your cluster at a later date, either manually or through the Alces Flight autoscaling feature).
