...
.. code:: bash
  ...
  [alces@login1(vlj) ~]$ bsub -w "done(103) && done(104)" < test.sh
...
The job script requests a total of 16 cores, requesting 2 cores on each compute host. The ``-R "span[ptile=2]"`` option can be used to specify the number of cores required per compute host. 
...
You can specify the maximum amount of memory required per submitted job with the ``-M [KB]`` option. This informs the scheduler of the memory required for the submitted job.
