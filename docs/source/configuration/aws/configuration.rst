...
The ``master`` field is used in compute node user-data only. Amazon VPC networks do not support multi-cast, so the Alces Compute image will not use its in-build discovery service, as such - you must provide the ``PrivateIP`` of the cluster master node in the user-data of each deployed compute node in order to correctly configure. 

Other options are available, such as using AWS:Route 53 and automatically configuring DNS entries. 
