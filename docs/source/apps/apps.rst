...
Users who are new to Linux may find it convenient to install applications on the login node only via a graphical session from the Linux application catalogue. Follow the steps below to enable the graphical Linux package installer:
 ...
 - Start a new graphical session if you don't already have one; ``alces session start gnome``
...
Alces Flight Compute clusters include access to the online **Gridware** repository of software applications. This catalogue includes over 750 application, library, compiler and MPI versions which can be installed and run on your cluster. Software is installed by selecting it from the catalogue, compiling it on the cluster login node along with any software pacakge dependancies, and installing it in the shared cluster application storage space. Once installed, applications can be run on the login and compute nodes interactively, or as part of job-scripts submitted to the cluster scheduler. 

Application catalogue structure
-------------------------------

Software applications are listed in the Alces Gridware repository in the structure ``status/type/name/version/varient``, which corresponds to:

 - **status** - packages are listed in the **main** repository if tested/stable, and the **volatile** repository otherwise. 
 - **type** - packages are listed as **apps** (applications), **libs** (shared libraries), **compilers** or **mpi** (message-passing interface APIs)
 - **name** - the name of the software package
 - **version** - the published version of the software package
 - **varient** - used at installation time to record information about a particular instance of a package, e.g. libraries used to build against, to arbitrary tags
...
..note:: By default, only the ``main`` respository is enabled; please read the instructions below to enable and use packages from the ``volatile`` repository. 
...
.. note:: Module names will auto-complete if the **<TAB>** button is pressed on the keyboard. 
...
.. note:: Please remember credit open-source application provides by providing a URL to the supporting project along with your research papers and publications.
...
Applications packaged in the ``main`` repository are tested to install without user interaction on an Alces Flight Compute cluster. For access to a larger catalogue of software, users can additionally enable the ``volatile`` software repository. Once enabled, advanced users can access the full list of available applications by choosing software along with any dependancies to install from the combined package list. 

To enable volatile repositories, edit the ``/opt/gridware/etc/gridware.yml`` YAML file and un-comment the volatile repository by removing the ``#`` symbol at the start of line XYZ. Alternatively, users can enable the repository by using the following command:
...
When installing packages from the volatile repo, users must resolve any dependancies before applications can be successfully installed. The Gridware packager will report any issues when attempting to install software from the volatile repo. The example below shows installation of the "beast" bio-informatics tool, which requires a Java Development Kit (JDK) to build:
...
The YUM utility can be used to identify any system packages which may satisfy build dependancies; e.g. 
...
Installing any dependancies may allow the software application to be installed as desired; e.g.
...
Alces Flight Compute clusters also support separate application depots which are preconfigured to include specific suites of applications for particular purposes. Depots can be used for the following puposes:
 ...
 - Collecting optimised applications together; e.g. those built with specialist accellerated compilers
 - Packaging your frequently used applications in a convenient bundle
 - Distributing your commercial applications (as permissable under the terms of the appropriate software license)
